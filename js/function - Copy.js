$(document).ready(function()
{
	...
	if ($('.pfwScroller').length > 0){
		...
		/*if (div2Height != $('.pfwScroller').position().top + $('.pfwScroller').height()) {
			...
			console.log('margin top: ' + div2Top);
...
function closeAlertBar(){
	...
	function openAlertBar(displayText, status = 'warning'){
		if(status == 'danger'){
			console.log("WARNING!!! RED");
			$('#alertBar').css('background-color' , 'rgb(220,20,60, 0.75)');
		} else if (status == 'succes') {
			$('#alertBar').css('background-color' , 'rgb(50,205,50, 0.75)');
		} else {
			$('#alertBar').css('background-color' , 'rgb(255,87,34, 0.75)');
		}
